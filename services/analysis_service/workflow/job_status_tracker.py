...
def mark_running(redis, report_id: str) -> None:
    update_pipeline_stage(redis, report_id, "ANALYSIS", "running", 86400)


def mark_success(redis, report_id: str) -> None:
    update_pipeline_stage(redis, report_id, "ANALYSIS", "completed", 86400)


def mark_failed(redis, report_id: str, error: str) -> None:
    update_pipeline_stage(redis, report_id, "ANALYSIS", "failed", 86400, {"error": error})
