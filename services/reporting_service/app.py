...
def _extract_ratio_rows(ratios: dict[str, Any]) -> list[list[str]]:
    metrics = [
        ...
        ("ROE", ratios.get("roe"), True),
        ("ROA", ratios.get("roa"), True),
        ...
        ("OCF to Net Profit", ratios.get("operating_cashflow_to_net_profit"), False),
        ("Asset Turnover", ratios.get("asset_turnover"), False),
        ("Financial Leverage", ratios.get("financial_leverage_ratio"), False),
        ("Cash Conversion Cycle", ratios.get("cash_conversion_cycle"), False),
    ]
...
def _write_report_pdf(report_id: str, payload: dict) -> str | None:
    ...
    # Trend chart block
    years_nm, vals_nm = _extract_trend_points(ratios, "net_margin")
    years_dr, vals_dr = _extract_trend_points(ratios, "debt_ratio")
    years_roe, vals_roe = _extract_trend_points(ratios, "roe")
    if len(vals_nm) >= 2 or len(vals_dr) >= 2 or len(vals_roe) >= 2:
        ...
        # Line chart for net margin and debt ratio.
...
async def generate_report(request: GenerateReportRequest):
    ...
    try:
        ...
        pdf_path = _write_report_pdf(report_id, report_payload)
        report_payload["pdf_report_path"] = pdf_path
