...
def mark_running(redis, report_id: str) -> None:
    update_pipeline_stage(redis, report_id, "REPORTING", "running", 86400)


def mark_success(redis, report_id: str) -> None:
    update_pipeline_stage(redis, report_id, "REPORTING", "completed", 86400)


def mark_failed(redis, report_id: str, error: str) -> None:
    update_pipeline_stage(redis, report_id, "REPORTING", "failed", 86400, {"error": error})
