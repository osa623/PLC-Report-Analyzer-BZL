...
class NormalizedIncomeRecord(BaseModel):
    report_id: str
    statement_type: Literal["income"]
...
class ExtractionStorePayload(BaseModel):
    report_id: str
    statement_type: Literal["income_statement"]
    status: Literal["completed", "partial", "failed"]
    normalized_rows: list[NormalizedIncomeRecord] = Field(default_factory=list)
