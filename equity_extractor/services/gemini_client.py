...
EQUITY_CHUNK_PROMPT = f"""
You are a forensic financial data extractor for Sri Lankan Public Listed Companies (PLCs).

YOUR GOAL:
Extract the **Statement of Changes in Equity** (SCE) line items from the provided text chunk.

{SHARED_CURRENCY_SCALE_INSTRUCTION}

SPECIFIC INSTRUCTIONS FOR SCE:
1. **Matrix Structure**:
   - The Statement of Changes in Equity is usually a matrix:
     - Columns: Share Capital, Revaluation Reserve, Retained Earnings, Total Equity.
     - Rows: Balance at beginning, Profit for the year, Dividend paid, Balance at end.
   - You must extract each cell value along with its row label and column header.

2. **Mandatory Line Items**:
   - "Balance as at 1 April ..." (Beginning Balance)
   - "Profit for the year" / "Total comprehensive income"
   - "Dividends paid"
   - "Balance as at 31 March ..." (Ending Balance)

3. **Column Headers**:
   - Capture the EXACT column header for each value.
   - Common headers: "Stated Capital", "Revaluation Reserve", "Retained Earnings", "Total".
...
{{
  ...
  "rows": [
    {{
      "label": "row label (e.g. Profit for the year)",
      "values": {{
        "Stated Capital": "0",
        "Retained Earnings": "12500",
        "Total": "12500"
      }},
      "section": "Equity Components|null",
      "parent_label": "null",
      "notes": "note number"
    }}
  ]
...
class GeminiExtractor:
    """Gemini API client for strict JSON extraction of SCE statements from text chunks."""
