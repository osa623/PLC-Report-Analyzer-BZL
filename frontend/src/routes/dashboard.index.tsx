...
function buildDashboardData(report: any, selectedYear: number) {
  ...
  const sortedYears = filterValidYears(byYear);
  ...
  const npmSeries = upToSelectedYears.map(y => byYear[y]?.net_profit_margin || 0);
  ...
  const kpisData = {
    ...
    roe: {
      ...
      series: roeSeries.length ? roeSeries : [0]
    },
    debtRatio: {
      value: `${((currentData.debt_ratio ?? 0) * 100).toFixed(2)}%`,
      change: calcChange(currentData.debt_ratio, prevData.debt_ratio),
      series: drSeries.length ? drSeries : [0]
    },
    netProfitMargin: {
      value: `${((currentData.net_profit_margin ?? 0) * 100).toFixed(2)}%`,
      change: calcChange(currentData.net_profit_margin, prevData.net_profit_margin),
      series: npmSeries.length ? npmSeries : [0]
    },
  };
