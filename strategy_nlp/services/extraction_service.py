...
class ExtractionService:
    ...
    def process(self, report_id: str, file_path: str) -> str:
        ...
        if not Path(file_path).is_file():
            ...
            raw_payload: dict = {}
            try:
                raw_payload = self.gemini_client.extract_strategy(file_path)
                strategy_count = len(raw_payload.get("strategies", [])) if isinstance(raw_payload, dict) else 0
                records, is_partial = self.transformation_service.transform(report_id, raw_payload)
                if is_partial:
                    status = "partial"
            except Exception as exc:
                logger.error(
                    "Strategy extraction failed for report_id=%s: %s",
                    report_id,
